...
class SpeakerClusterAnalyzer:
    ...
    def load(self):
        if not self.loaded:
            ...
            json_file = open(os.path.join(os.path.dirname(__file__), '../../weights/sound/cluster/model.json'), 'r')
            loaded_model_json = json_file.read()
            json_file.close()
            self.model = model_from_json(loaded_model_json)
            # We load the weights into our model
            self.model.load_weights(
                os.path.join(os.path.dirname(__file__), "../../weights/sound/cluster/weights.h5"))
            self.loaded = True
